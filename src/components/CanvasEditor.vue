...
export default {
  ...
  emits: ['update:value', 'contentChange', 'rangeStyleChange'],
  setup(props, { emit }) {
    ...
    onMounted(() => {
      ...
      if (editorRef.value) {
        const defaultOptions = {
          mode: 'edit', // 或者 'readonly'
          defaultType: 'canvas', // 或者 'svg'
        ...
        // 初始化编辑器并传入内容
        editor = new Editor(editorRef.value, editorOptions)

        // 如果有初始值，设置内容
        if (props.value && props.value.length) {
          editor.command.executeSetValue(props.value)
        }

        // 监听内容变化
        editor.listener.contentChange((data) => {
          emit('update:value', data)
          emit('contentChange', data)
        })

        // 监听选区样式变化
        editor.listener.rangeStyleChange((data) => {
          emit('rangeStyleChange', data)
        })
      }
    })
    ...
    // 暴露编辑器实例的关键方法
    const editorMethods = {
      getValue: () => editor?.command.getValue(),
      getHTML: () => editor?.command.getHTML(),
      getText: () => editor?.command.getText(),
      executeFormat: () => editor?.command.executeFormat(),
      executeBold: () => editor?.command.executeBold(),
      executeItalic: () => editor?.command.executeItalic(),
      // 其他需要的方法...
    }
